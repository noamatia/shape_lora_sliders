...
parser.add_argument("--name", type=str, default='spice_with_prompts_masking_percentile_90_beta_0_5', help="Name of the run.")
...
parser.add_argument("--spice_model_path", type=str, default="/scratch/noam/spice_models/semantic_editing_chair/best_model.pt", help="path to spice model")
...
def build_name_and_paths() -> tuple:
    ...
    output_dir = os.path.join('/scratch/noam/shape_lora_sliders', name)
...
def build_latents(test_or_train: str, folder:str ) -> list:
    latents = os.listdir(os.path.join(args.data_dir, test_or_train, folder))
...
def load_latents(test_or_train: str, folder: str, device: torch.device, latents: list) -> list:
    lats = [torch.load(os.path.join(args.data_dir, test_or_train, folder, latent)).to(device) for latent in latents]
...
def build_model_kwargs_list(prompt: str, latents: list, test_or_train: str, low_or_high: str, loaded_latents: list = None) -> tuple:
    if args.uid_to_object:
        ...
        with open(os.path.join(args.data_dir, test_or_train, f'{args.uid_to_object}.json'), 'r') as f:
...
def decode_and_export_test_conditions(model_kwargs_high_test_list, model_kwargs_low_test_list, paths, cameras, xm):
    for model_kwargs_high_test, model_kwargs_low_test in tqdm(zip(model_kwargs_high_test_list, model_kwargs_low_test_list), 
        ...
        for i in range(args.batch_size):
            ...
            # decode_and_export(model_kwargs_high_test['cond'][i], uid_output_dir_high, f'condition.gif', cameras, xm)
            # decode_and_export(model_kwargs_low_test['cond'][i], uid_output_dir_low, f'condition.gif', cameras, xm)
